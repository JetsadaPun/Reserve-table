...
public class Book extends JFrame implements ActionListener, Finally, setComponent, Initial {
    Container cp;
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getActionCommand() == "Save") { // ถ้ากดปุ่ม Save 
            String name = t1.getText(); //ดึงชื่อจาก t1 เก็บในตัวแปร name
            String phone = t2.getText(); //ดึงเบอร์โทรศัพท์ จาก t2 เก็บในตัวแปร phone
            String partySize = t3.getText(); //ดึงจำนวนคนจาก t3 เก็บในตัวแปร partySize
            if (name.isEmpty() || phone.isEmpty() || partySize.isEmpty()) { //ตรวจสอบว่าชื่อ เบอร์โทร และจำนวนคนไม่เป็นค่าว่าง ถ้าเป็นค่าว่างให้แจ้งเตือนให้กรอกข้อมูลให้ครบ
                JOptionPane.showMessageDialog(this, "Please fill out all fields.", "ข้อมูลไม่ครบ",
                        JOptionPane.WARNING_MESSAGE);
            } else if (phone.matches("\\d+") && phone.length() >= 10) { //ตรวจสอบว่าเบอร์โทรศัพท์มีแต่ตัวเลขและความยาวอย่างน้อย 10 ตัว
                table table = new table(name, phone, partySize, (String) c1.getSelectedItem(),
                        (String) c2.getSelectedItem());
                table.setVisible(true); //ถ้าเงื่อนไขถูกต้องจะแสดงหน้า table 
                dispose(); //และปิดหน้า Book
            }
        } else if (e.getActionCommand() == "Back") { //ถ้ากดปุ่ม Back จะแสดงหน้า Login และปิดหน้า Book
